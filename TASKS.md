...
- [ ] clamp-worker-downscale-constants: `photo-worker.js` `onmessage` trusts `maxDimension`/`quality` from the message with no range-clamp. Not exploitable (same-realm trusted dispatcher passes the module constants), but a one-line guard (`Number.isFinite` + `Math.min/Math.max`) would harden against a future caller bug. (surfaced by offload-photo-downscale-to-worker SECURITY-REVIEW)
