...
The site renders from `data/days.js`. **12 days are authored (Jun 16–17 + Jun 24 – Jul 3).** The **Jun 18–23** leg (Tokyo through Jun 22, Hakone Jun 22–24) is not in the data yet — it's owed by a friend and gets added later. The scaffold renders the partial set gracefully (absent days return `null`, no crash). `dayNumber` is derived from `TRIP.start` (Jun 16 = Day 1), so the authored days are Days 1–2 (Jun 16–17) and Days 9 → 18 (Jun 24 – Jul 3).
...
| `sw.js` | Hand-written service worker (no build step) — precaches the app shell into `app-shell-v<CACHE_VERSION>` (currently `v14`), runtime-caches photos/fonts into `runtime-v<CACHE_VERSION>`. Bump `CACHE_VERSION` when shell files change. `test.html` is intentionally NOT precached (dev tool, network-only). |
...
| `app.test.js` | `node:test` cases for the data, render, date/time-nav, time-travel, pre-trip-home, collapsible day-parts, structured-transit, and rec transit-alternative pill layers (**263 total with `sw.test.js`**). Run with `node --test`. |
...
- `getDay(iso)` → the day for an ISO date, or `null` if absent (e.g. the Jun 18–23 gap).
...
- The **Jun 18–23 days** are not yet authored in `data/days.js` (content owed) — rendered as an absent range by design.
