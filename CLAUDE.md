...
| `sw.js` | Hand-written service worker (no build step) — precaches the app shell into `app-shell-v<CACHE_VERSION>` (currently `v28`), runtime-caches photos/fonts/Firebase-SDK into `runtime-v<CACHE_VERSION>` (FIFO-capped at `RUNTIME_MAX_ENTRIES`, currently 140 — raised from 60 for the live photo journal). Bump `CACHE_VERSION` when shell files change. `test.html` is intentionally NOT precached (dev tool, network-only). |
