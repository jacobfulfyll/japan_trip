...
| `app.test.js` | `node:test` cases for the data, render, date/time-nav, time-travel, pre-trip-home, collapsible day-parts, structured-transit, rec transit-alternative pill, and reminisce-gallery layers (**309 total with `sw.test.js`**). Run with `node --test`. |
