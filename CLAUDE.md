...
| `sw.js` | Hand-written service worker (no build step) — precaches the app shell into `app-shell-v<CACHE_VERSION>` (currently `v18`), runtime-caches photos/fonts into `runtime-v<CACHE_VERSION>`. Bump `CACHE_VERSION` when shell files change. `test.html` is intentionally NOT precached (dev tool, network-only). |
...
| `app.test.js` | `node:test` cases for the data, render, date/time-nav, time-travel, pre-trip-home, collapsible day-parts, structured-transit, and rec transit-alternative pill layers (**301 total with `sw.test.js`**). Run with `node --test`. |
...
- The trip is fully authored (18/18 days, Jun 16 – Jul 3, contiguous). The `UNAUTHORED_REGIONS` fallback map in `app.js` is now dead (all its dates have authored `day.base`) — safe to drop in a cleanup pass.
