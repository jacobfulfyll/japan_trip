...
| `sw.js` | Hand-written service worker (no build step) — precaches the app shell into `app-shell-v<CACHE_VERSION>` (currently `v24`), runtime-caches photos/fonts/Firebase-SDK into `runtime-v<CACHE_VERSION>` (FIFO-capped at `RUNTIME_MAX_ENTRIES`, currently 60). Bump `CACHE_VERSION` when shell files change. `test.html` is intentionally NOT precached (dev tool, network-only). |
...
| `app.test.js` | `node:test` cases for the data, render, date/time-nav, time-travel, pre-trip-home, collapsible day-parts, structured-transit, rec transit-alternative pill, and reminisce-gallery/lightbox, and auth-gate layers (**342 total with `sw.test.js`**). Run with `node --test`. |
